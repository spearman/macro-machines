...
macro_rules! def_machine {
  ...
  ( @impl_fn_dotfile
    ...
    /// &#9888;: this function constructs default values from default
...
macro_rules! def_machine_debug {
  ...
  ( @impl_fn_dotfile
    ...
    fn _dotfile (
      ...
      // transitions
      s.push_str (
        def_machine_debug!(@fn_dotfile_transition_initial $initial
      ).as_str());
      $(
      s.push_str (def_machine_debug!(
        @fn_dotfile_transition
      ...
      // terminal
      $(
      s.push_str (def_machine_debug!(@fn_dotfile_node_terminal).as_str());
      s.push_str (def_machine_debug!(
        @fn_dotfile_transition_terminal $terminal
      ).as_str());
      )*
      ...
      //  end graph
      s.push_str (def_machine_debug!(@fn_dotfile_end).as_str());
      s
    } // end fn dotfile
  ...
  //
  //  @impl_fn_dotfile_nodefault_debug
  //
  ( @impl_fn_dotfile_nodefault_debug
    machine $machine:ident $(<$($type_var:ident),+>)* {
      STATES [
